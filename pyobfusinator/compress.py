...
def unicode_compress(input_code: str) -> bytes:
    if '\t' in input_code:
        warnings.warn("Code cannot contain tabs. Converting each tab to 4 spaces.", stacklevel=2)
    ...
    return b"b='" + o + b"'.encode();exec(''.join(chr(((h<<6&64|c&63)+22)%133+10)for h,c in zip(b[1::2],b[2::2])))"
